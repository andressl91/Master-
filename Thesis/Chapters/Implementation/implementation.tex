\chapter{Discretization of Fluid Structure Interaction}

The aim of this chapter is to present some of the main challenges regarding discretization of a general monolithic fluid-structure interaction(FSI) problem, using the ALE-framework. The general FSI problem consist of coupling the fluid and solid governing equations together with the interface conditions. Even separately, the discretization of fluid and structure problems impose rather difficult issues due to their non-linear nature. However, their long-time existence within research community makes them well known problems and a vast number of rigorous approaches and commercial  software exist to solve them individually. When solving the fluid and structure simultaneously however, the overall problem gets more complex due to the overall dependency of the two sub-problems and their interaction to one another. The most common approach to couple the governing equations is the ALE-framework. 

The main goal of the ALE-framework is to map the fluid problem from an Eulerian reference system to a Lagrangian reference system. However the ALE-framework impose two main constrains to the overall system. 

Firstly, the choice of extrapolation method of the structure velocity is essential to ensure consistent fluid computations. Since one often choose a model which is familiar(simple to solve), the extrapolation problem itself is not the problem. The main issue is the produced mesh deformation produced for a given model. Therefore the problem is not the discretization, but the ability to produce a good mesh without overlapping cells. Therefor it is not a issue for the computation itself, but which the foundadtion for which the fluid problem is solved on. It will not be discussed in this chapter..  

Second, the ALE-framework introduces the domain-velocity term to the convection term in the fluid problem. 


Compared to the other mentioned non-linear effects, this
