...
\subsection{ALE formulation of the fluid problem}
 Recall the incompressible Navier-Stokes equations are defined in an \textit{Eulerian description of motion}, $\Omega_f (t)$.
\begin{align*}
&\rho \pder{\mathbf{v}}{t} + \rho \mathbf{v} \cdot \nabla \mathbf{v} =
\nabla \cdot \sigma + \rho \mathbf{f} \\
&\nabla \cdot \mathbf{v} = 0
\end{align*}
which no longer comply when changing the computational domain to an underformed \textit{reference configuration} $\hat{\Omega}_f (t)$. By using the fluid domain map $\bat{T}_f (t)$, one can transform the original Navier-Stokes equations onto the  \textit{reference configuration} $\hat{\Omega}_f (t)$.
Introducing the basic properties needed for mapping between the sub-system $\hat{\Omega}_f (t)$ and $\Omega_f (t)$, we will present the ALE  time and space derivative transformations found in \cite{Richter2016}, with help of a new arbitrary fixed reference system \ha{W} .  Further we denote its deformation gradient as $\hat{F}_w$ and its determinant $\ha{J}_w$. We introduce the invertable mapping $\ha{T}_w : \ha{W} \rightarrow V(t)$ , with the scalar $\ha{f}(\ha{x}_W, t) = f(x,t) $ and vector $\bat{w}(\ha{x}_W, t) = \mathbf{w}(x,t) $ counterparts. 
...
Using our newly introduced transformations of derivatives we map the equation to the arbitrary reference system $\ha{W}$.
